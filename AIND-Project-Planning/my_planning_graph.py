...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # add a new level in a_levels
        self.a_levels.append(set())
        for action in self.all_actions:
            a_node = PgNode_a(action)
            # Pick the action whose preconditions fit the states in s_levels[level]
            if a_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(a_node)
                for s_node in self.s_levels[level]:
                    # get the state node that meets the preconditions in action node
                    if s_node in a_node.prenodes:
                        # add s_node to action node's parent
                        a_node.parents.add(s_node)
                        # add a_node to s_node's children
                        s_node.children.add(a_node)
                # add effect nodes to action's children
                a_node.children = a_node.effnodes
                # add the action node to this level
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_precond in node_a1.action.precond_pos:
            if pos_precond in node_a2.action.effect_rem:
                is_interference = True
                break
        for neg_precond in node_a1.action.precond_neg:
            if neg_precond in node_a2.action.effect_add:
                is_interference = True
                break
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for s_node_to_a1 in node_a1.parents:
            for s_node_to_a2 in node_a2.parents:
                if s_node_to_a1.is_mutex(s_node_to_a2):
                    is_competing = True
                    return is_competing
        return is_competing
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        is_negation = (node_s1.symbol == node_s2.symbol) and (node_s1.is_pos != node_s2.is_pos)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        is_inconsistent_support = True

        if len(node_s1.parents) == len(node_s2.parents):
            for a_node_to_s1 in node_s1.parents:
                for a_node_to_s2 in node_s2.parents:
                    if not a_node_to_s1.is_mutex(a_node_to_s2):
                        is_inconsistent_support = False
                        return is_inconsistent_support
        else:
            is_inconsistent_support = False
        ...
        for goal_literal in self.problem.goal:
            ...
            for i, s_level in enumerate(self.s_levels):
                ...
                for s_node in s_level:
                    if goal_literal == s_node.symbol and s_node.is_pos:
                        level_sum += i
                        found = True
                        break
        return level_sum
